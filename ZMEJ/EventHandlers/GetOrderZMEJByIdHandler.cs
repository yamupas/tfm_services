...
{
    public class GetOrderZMEJByIdHandler : IRequestHandler<GetOrderZMEJByIdQuery, OrderZMEJDto>
    {
        private IOrderZMEJRepository _orderZMEJRepository;
        private IIdentityService _identityService;
        public GetOrderZMEJByIdHandler(IOrderZMEJRepository orderZMEJRepository, IIdentityService identityService)
        {
            _orderZMEJRepository = orderZMEJRepository;
            _identityService = identityService;
        }
        public async Task<OrderZMEJDto> Handle(GetOrderZMEJByIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                return data != null ? new OrderZMEJDto
                {
                    ...
                    DuraciondelTrabajo=data.DuraciondelTrabajo,
                    valComprometido=data.valComprometido
